    ...
    for (const task of tasks) {
        html += `<article class="rounded-3xl border border-blue-100 bg-white p-6 shadow-lg shadow-blue-50">
                    <div class="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
                        <div>
                            <p class="task-state text-xs font-bold uppercase tracking-[0.25em] text-blue-600">${task.status} - ${task.date}</p>
                            <h2 class="task-title mt-2 text-2xl font-bold text-slate-900">${task.title} - ${task.userId}</h2>
